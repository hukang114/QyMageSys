...
public class StatusBarUtil {
     ...
     * @param statusBarAlpha 状态栏透明度
     */
    public static void setColor1(Activity activity, int statusBarAlpha, int rgb1, int rgb2, int rgb3) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            activity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            activity.getWindow().clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            activity.getWindow().setStatusBarColor(Color.argb(statusBarAlpha,rgb1,rgb2,rgb3));
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            activity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            ViewGroup decorView = (ViewGroup) activity.getWindow().getDecorView();
            View fakeStatusBarView = decorView.findViewById(FAKE_STATUS_BAR_VIEW_ID);
            if (fakeStatusBarView != null) {
                ...
                fakeStatusBarView.setBackgroundColor(Color.argb(statusBarAlpha,rgb1,rgb2,rgb3));
            } else {
                decorView.addView(createStatusBarView(activity, statusBarAlpha, statusBarAlpha));
            }
            setRootView(activity);
        }
    }
